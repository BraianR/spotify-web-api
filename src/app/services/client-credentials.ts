...
export class ClientCredentialsService {
  ...
  sendAuthRequest(): Observable<SpotifyAuthResponse> {
    ...
    const headers = new HttpHeaders({
      Authorization: 'Basic ' + btoa(`${spotifyApiKeys.CLIENT_ID}:${spotifyApiKeys.CLIENT_SECRET}`),
      'Content-Type': 'application/x-www-form-urlencoded',
    });

    return this.http
      .post<SpotifyAuthResponse>('https://accounts.spotify.com/api/token', body.toString(), {
        headers,
      })
